(() => {
  ...
  function calculate() {
    ...
    result.projection = result.storeDailyAvg * num(db.businessDays) + num(db.ecommerce) - num(db.returns);
  ...
  function renderOverview() {
    ...
    const conversion = result.general ? result.eligible / result.general : 0;
    setText('efficiencyKpi', efficiencyPct.format(result.efficiency)); setText('conversionKpi', result.general ? efficiencyPct.format(conversion) : 'Não calculada'); setText('ticketKpi', brl.format(result.ticket));
    ...
    if (showEcommerce) {
      const ecommerce = num(db.ecommerce), returns = num(db.returns), consolidated = result.revenue;
      setText('storeSalesOverview', brl.format(result.general));
      setText('ecommerceSalesOverview', brl.format(ecommerce));
      setText('returnsOverview', brl.format(returns));
  ...
  function renderWeekly() {
    ...
    grid.innerHTML = weekBuckets().map((items, index) => {
      ...
      const conversion = result.general ? result.eligible / result.general : 0;
      const conversionStatus = result.general ? (conversion >= 0.35 ? 'passed' : 'failed') : '';

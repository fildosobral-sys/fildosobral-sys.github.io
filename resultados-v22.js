(() => {
  ...
  function compiledTemporalAnalysis() {
    ...
    const totals = points.reduce((sum, point) => ({ sales: sum.sales + point.sales, services: sum.services + point.services, grossProfit: sum.grossProfit + point.grossProfit, target: sum.target + point.target, grossTarget: sum.grossTarget + point.grossTarget, serviceTarget: sum.serviceTarget + point.serviceTarget, worked: sum.worked + point.worked, nfs: sum.nfs + point.nfs, invoiceCount: sum.invoiceCount + point.invoiceCount }), { sales: 0, services: 0, grossProfit: 0, target: 0, grossTarget: 0, serviceTarget: 0, worked: 0, nfs: 0, invoiceCount: 0 });
  ...
  function renderTierBalanceDashboard(data) {
    ...
    document.getElementById('periodTierDashboard').innerHTML = tiers.map((tier) => {
      ...
      return `<article class="tier-balance-card"><header><strong>${tier.name}</strong><span class="${statusClass(overall)}">${pct.format(overall)}</span></header><div class="tier-balance-row"><span>Mercantil realizado / meta</span><strong>${brl.format(totals.sales)} / ${brl.format(tier.mercantile)}</strong></div><div class="tier-balance-row"><span>Saldo mercantil acumulado</span><strong class="${mercBalance >= 0 ? 'positive' : 'negative'}">${signedMoney(mercBalance)}</strong></div><div class="tier-balance-row"><span>Lucro bruto realizado / meta</span><strong>${totals.grossProfit ? `${brl.format(totals.grossProfit)} / ${brl.format(tier.gross)}` : 'Não informado'}</strong></div><div class="tier-balance-row"><span>Saldo de lucro acumulado</span><strong class="${grossBalance >= 0 ? 'positive' : 'negative'}">${totals.grossProfit ? signedMoney(grossBalance) : '—'}</strong></div><div class="tier-needed ${passed ? 'positive' : ''}"><strong>${passed ? 'Meta coberta no período' : `Necessário até ${finalLabel}`}</strong><div>Mercantil: ${brl.format(needMerc)}${totals.grossProfit ? ` • Lucro: ${brl.format(needGross)}` : ''}</div></div></article>`;
    }).join('');
  }

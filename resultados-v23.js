(() => {
  ...
  function renderSellers() {
    ...
    list.querySelectorAll('.seller-row').forEach((row) => {
      ...
      const legacyNfsField = fields.querySelector('[data-f="nfs"]')?.closest('.seller-field');
      if (legacyNfsField?.querySelector('label')) legacyNfsField.querySelector('label').textContent = 'Quantidade elegível do período';
      const invoiceField = document.createElement('div'); invoiceField.className = 'seller-field';
      invoiceField.innerHTML = `<label>Notas fiscais do período</label><input inputmode="numeric" type="number" min="0" step="1" data-f="invoiceCount" value="${num(seller.invoiceCount) || ''}">`;
      legacyNfsField?.insertAdjacentElement('beforebegin', invoiceField);
      const warrantyValueField = fields.querySelector('[data-f="warranty"]')?.closest('.seller-field');
      const warrantyQtyField = document.createElement('div'); warrantyQtyField.className = 'seller-field';
      warrantyQtyField.innerHTML = `<label>Quantidade de garantias vendidas</label><input inputmode="numeric" type="number" min="0" step="1" data-f="warrantyQty" value="${num(seller.warrantyQty) || ''}">`;
      warrantyValueField?.insertAdjacentElement('afterend', warrantyQtyField);
      const conversionMetric = row.querySelector('.seller-card-content > .seller-metrics .metric:last-child');
      if (conversionMetric) conversionMetric.innerHTML = `<span>CONVERSÃO • META 35%</span><strong>${num(seller.nfs) ? efficiencyPct.format(num(seller.warrantyQty) / num(seller.nfs)) : 'Não calculada'}</strong><small>${num(seller.warrantyQty)} garantia(s) ÷ ${num(seller.nfs)} elegível(is)</small>`;
      ...
      periodBlock.querySelector('.seller-fixed-targets').innerHTML = '<span>Eficiência de serviços: 7%</span><span>Meta de conversão: 35%</span><span>Conversão: quantidade de garantias ÷ quantidade elegível</span>';
  ...
  function renderTierBalanceDashboard(data) {
    const totals = data.totals, finalLabel = monthLabel(data.reference);
    const sellerCount = Math.max(1, num(data.points.at(-1)?.sellerCount) || configuredSellerCount());
    const tiers = [
    ...
    document.getElementById('periodTierDashboard').innerHTML = tiers.map((tier) => {
      const mercBalance = totals.sales - tier.mercantile, grossBalance = totals.grossProfit - tier.gross;
      const mercRate = tier.mercantile ? totals.sales / tier.mercantile : 0, grossRate = tier.gross ? totals.grossProfit / tier.gross : 0;
      const overall = totals.grossProfit > 0 ? Math.min(mercRate, grossRate) : mercRate;
      const needMerc = Math.max(0, -mercBalance), needGross = Math.max(0, -grossBalance), passed = needMerc === 0 && (totals.grossProfit <= 0 || needGross === 0);
      const storeDaily = totals.worked ? totals.sales / totals.worked : 0;
      const targetDaily = totals.plannedDays ? tier.mercantile / totals.plannedDays : 0;
      const dailyBalance = storeDaily - targetDaily;
      const sellerDailyBalance = dailyBalance / sellerCount;
      const dailySituation = (value) => value >= 0 ? `Vendeu a mais ${brl.format(value)}` : `Faltou ${brl.format(Math.abs(value))}`;
      const signedMoney = (value) => `${value >= 0 ? '+' : '−'} ${brl.format(Math.abs(value))}`;
      return `<article class="tier-balance-card"><header><strong>${tier.name}</strong><span class="${statusClass(overall)}">${pct.format(overall)}</span></header><div class="tier-balance-row"><span>Mercantil realizado / meta</span><strong>${brl.format(totals.sales)} / ${brl.format(tier.mercantile)}</strong></div><div class="tier-balance-row"><span>Saldo geral acumulado</span><strong class="${mercBalance >= 0 ? 'positive' : 'negative'}">${dailySituation(mercBalance)}</strong></div><div class="tier-balance-row"><span>Venda média/dia da loja</span><strong>${brl.format(storeDaily)}</strong></div><div class="tier-balance-row"><span>Meta média/dia da loja</span><strong>${brl.format(targetDaily)}</strong></div><div class="tier-balance-row"><span>Saldo da loja por dia</span><strong class="${dailyBalance >= 0 ? 'positive' : 'negative'}">${dailySituation(dailyBalance)}</strong></div><div class="tier-balance-row"><span>Saldo por vendedor/dia</span><strong class="${sellerDailyBalance >= 0 ? 'positive' : 'negative'}">${dailySituation(sellerDailyBalance)}</strong></div><div class="tier-balance-row"><span>Equipe considerada</span><strong>${sellerCount} vendedor(es)</strong></div><div class="tier-balance-row"><span>Lucro bruto realizado / meta</span><strong>${totals.grossProfit ? `${brl.format(totals.grossProfit)} / ${brl.format(tier.gross)}` : 'Não informado'}</strong></div><div class="tier-balance-row"><span>Saldo de lucro acumulado</span><strong class="${grossBalance >= 0 ? 'positive' : 'negative'}">${totals.grossProfit ? signedMoney(grossBalance) : '—'}</strong></div><div class="tier-needed ${passed ? 'positive' : ''}"><strong>${passed ? 'Meta coberta no período' : `Necessário até ${finalLabel}`}</strong><div>Mercantil geral: ${brl.format(needMerc)}${totals.grossProfit ? ` • Lucro: ${brl.format(needGross)}` : ''}</div></div></article>`;
    }).join('');
  }
